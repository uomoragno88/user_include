...
; Parameters ....: $oOL         - Outlook object returned by a preceding call to _OL_Open()
;                  $vRecipients - Name, Alias or SMTP mail address of one or multiple recipients separated by ";"
; Return values .: Success - two-dimensional one based array with the following information:
...
Func _OL_VersionInfo()
	...
	Local $aVersionInfo[9] = [8, "V", 1, 0, 0.0, "20140727", "3.3.10.2", "wooltown, water", _
...
Func __OL_CheckProperties($oItem, $aProperties, $iFlag = 0, $bItemProperties = True)
	...
	For $iIndex = $iIndex To $iEnd
		...
		If $aTemp[1] <> "" Then
			...
			If Not ($aTemp[1] == $oProperty.Name) Then SetError(Int(11 & StringRight("0" & $iIndex, 2)), 0, 0) ; Case doesn't match
		EndIf
	Next
	Return 1
